...
def generate_dither(num_coeffs, delta=10, seed=42):
    rng = np.random.default_rng(seed)
    return rng.uniform(-delta/4, delta/4, size=num_coeffs)

def quantize(coeff, delta=10, bit=0, dither=0.0):
    if bit == 0:
        return delta * np.round((coeff + dither) / delta) - dither
    else:
        return delta * np.round((coeff + dither - delta/2) / delta) + delta/2 - dither

def embed_qim_dither(input_wav, output_wav, watermark_bits, delta=10, frame_size=1024, overlap=512, key=42):
...
def extract_qim_dither(watermarked_wav, watermark_length, delta=10, frame_size=1024, overlap=512, key=42):
...
if __name__ == "__main__":
    ...
    embed_qim_dither(input_audio, output_audio, watermark_bits, delta, frame_size, overlap)
    extracted_bits = extract_qim_dither(output_audio, len(watermark_bits), delta, frame_size, overlap)
    ...
    # print(f"Extracted audio watermark saved to {extracted_watermark}")