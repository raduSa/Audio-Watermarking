...
def quantize(coeff, delta, bit):
    if bit == 0:
        return delta * np.round(coeff / delta)
    else:
        return delta * np.round((coeff - delta / 2) / delta) + delta / 2

def embed_qim(input_wav, output_wav, watermark, watermark_type, delta, frame_size, overlap):
...
def extract_qim(watermarked_wav, watermark_length, watermark_type, delta, frame_size, overlap):
